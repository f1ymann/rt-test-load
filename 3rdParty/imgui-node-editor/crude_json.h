...
namespace detail {
    ...
    template <typename T>
    inline bool GetPtrTo(const crude_json::value& value, std::string_view key, const T*& result)
    {
        std::string keyString = key.data();
...
# endif
    ...
    inline bool GetTo(const crude_json::value& value, std::string_view key, V& result)
    {
