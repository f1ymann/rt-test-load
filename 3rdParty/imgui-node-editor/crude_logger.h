...
# define FMT_pin(pin)       "Pin", (pin)->ID.Get(), (pin)->Name.empty() ? "" : " \"", FMT_sv((pin)->Name), (pin)->Name.empty() ? "" : "\""

# define PRI_node           "s %" PRIu32 "%s%" PRI_sv "%s"
# define FMT_node(node)     "Node", (node)->ID.Get(), (node)->Name.empty() ? "" : " \"", FMT_sv((node)->Name), (node)->Name.empty() ? "" : "\""
